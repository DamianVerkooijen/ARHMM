...
public class IntroSequenceController : MonoBehaviour
{
    ...
    private IEnumerator FadeOutAndStartGame()
    {
        ...
        MissionStateController.PopupContent introPopup =
            firstMission.missionIntroPopup;
        ...
        if (introPopup != null)
        {
            if (!string.IsNullOrEmpty(introPopup.title))
                title = introPopup.title;

            if (!string.IsNullOrEmpty(introPopup.description))
                description = introPopup.description;

            if (introPopup.icon != null)
                icon = introPopup.icon;

            if (!string.IsNullOrEmpty(introPopup.actionButtonText))
                actionLabel = introPopup.actionButtonText;
        }
